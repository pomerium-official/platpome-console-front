...
export function focusFirstInvalidElement(
  ...
  if (firstInputRef) {
    ...
    if (typeof firstInputRef.focus !== 'undefined') {
      firstInputRef.focus();
      return;
    }

    // primereact Button Focus처리
    if (
      firstInputRef.input &&
      typeof firstInputRef.input.focus !== 'undefined'
    ) {
      firstInputRef.input.focus();
      return;
    }

    // primereact Input Focus처리
    if (
      firstInputRef.element &&
      typeof firstInputRef.element.focus !== 'undefined'
    ) {
      firstInputRef.element.focus();
      return;
    }

    // primereact Password v1 Focus처리
    if (
      firstInputRef.inputEl &&
      typeof firstInputRef.inputEl.focus !== 'undefined'
    ) {
      firstInputRef.inputEl.focus();
      return;
    }

    // primereact Password v2 Focus처리
    if (
      firstInputRef.inputRef &&
      firstInputRef.inputRef.current &&
      typeof firstInputRef.inputRef.current.focus !== 'undefined'
    ) {
      firstInputRef.inputRef.current.focus();
      return;
    }
    ...
    if (
      firstInputRef.quill &&
      typeof firstInputRef.quill.focus !== 'undefined'
    ) {
      firstInputRef.quill.focus();
      return;
    }
