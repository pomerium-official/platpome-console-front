...
export const isLoginFreePath = (path: string) => {
  if (process.env.NEXT_PUBLIC_LOGIN_FREE === 'true') {
    return true;
  }
  const loginFreePaths = process.env.NEXT_PUBLIC_PUBLIC_PATHS!.split(',');

  return loginFreePaths.some((s) => {
...
export const prettyJson = (
  ...
  if (jsonStr) {
    let result = jsonStr;
    try {
      result = JSON.stringify(JSON.parse(jsonStr), undefined, space);
    } catch (e) {
      return jsonStr;
    }
    console.warn('pretty', result);
    return result;
  }
  return '';
};
